...
import { createStudentAction, deleteStudentAction, getStudentsAction, updateStudentAction } from "./students.actions";
...
export default function StudentsManagement() {
  ...
  const createMutation = useMutation({
    mutationFn: (payload: object) => createStudentAction(payload),
    onSuccess: async (res) => {
      if (res.success) {
        queryClient.invalidateQueries({ queryKey: ["students"] });
        await showSuccess("Student added! ✅");
        resetForm();
      } else {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, payload }: { id: string; payload: object }) =>
      updateStudentAction(id, payload),
    onSuccess: async (res) => {
      if (res.success) {
        queryClient.invalidateQueries({ queryKey: ["students"] });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-bold text-gray-800 dark:text-gray-100">🗣️ Students</h2>
        <button
      ...
      {/* ── Modal ── */}
      <StudentsModal        isOpen={isModalOpen}
        onClose={resetForm}
        formData={formData}
